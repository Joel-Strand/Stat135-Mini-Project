...
#A few gaps may indicate that the model is unimodal but the relative distribution is symmetric and given our sample size our inferences should still be alright due to the central limit theorem. 
...
for (lane in lanes) { # for loop for predicting each lane
  ...
  predictions_RL <- c(predictions_RL, val) # store predictions in a list for later access.
}
